...
void
UdpClient::SetPacketTag(uint32_t tag_val){
  NS_LOG_FUNCTION (this << tag_val);
  m_tag_val = tag_val;
}

void
UdpClient::SetOutputPath(uint8_t output){
...
UdpClient::Send (void)
{
  ...
  std::string str = peerAddressStringStream.str();
  std::stringstream str2;
  str2 << str;
  while(str2.str().length() < 10)
    str2 << " ";
  const char* strp = str2.str().c_str();
  Ptr<Packet> p = Create<Packet> (reinterpret_cast<const uint8_t*>(strp), m_size-(8+4)); // 8+4 : the size of the seqTs header

  //Ptr<Packet> p = Create<Packet> (m_size-(8+4)); // 8+4 : the size of the seqTs header
  p->AddHeader (seqTs);

  if(m_tag_val != 0){
    uint32_t temp = m_tag_val;
    if(m_output){
      temp |= m_output << 24;
      m_output = 0;
    }
    MyTag tag;
    tag.SetSimpleValue(temp);
    p->AddPacketTag(tag);
  }
