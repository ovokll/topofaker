...
int TOPO_INDEX = 1;//1 dfn  2 cogentco  3deltacom
int precent_index = 0;//0-9

int add_fake_delay = 1;// 0: not add    1: add delay
int fake_delay_mode = 1;//1: topoFaker
int experiment_index = 2;
int random_avg = 16;
int random_var = 20;

unsigned int pkt_base_len = 56+30;
int one_hop_delay = 6; // first hop：11，other hop：5
int edge_host_nums[3][10] = {{5, 6, 8, 9, 11, 12, 14, 15, 17, 18},
                             {9, 15, 21, 26, 32, 38, 43, 49, 55, 60}, 
                             {7, 10, 13, 16, 19, 22, 25, 28, 31, 34}};
bool is_init = false;
int host_num = edge_host_nums[TOPO_INDEX-1][precent_index];
int max_index = host_num*(host_num-2)*(host_num-1)/2;
int proto_max_index = (host_num-2)*(host_num-1)/2;
std::ifstream file;
std::normal_distribution<> dist;
std::mt19937 gen;
int max_delay = 0;

void get_source_and_dest_addr(int pkt_index, ns3::Ipv4Address addr[]){
  int i = pkt_index / ((host_num-2)*(host_num-1)/2);
  int left = pkt_index % ((host_num-2)*(host_num-1)/2) + 1;
  int j = 0, max = (host_num*2-4-j) * (j+1)/2;
  while(left > max){
    j++;
    max = (host_num*2-4-j) * (j+1)/2;
  }
  
  if(j >= i)
    j++;

  std::string str1 = "10.2.0.", str2 = "10.2.0.";
  int num1 = i*2+1, num2 = j*2+1;
  std::stringstream ss1, ss2;
  ss1 << str1 << num1;
  ss2 << str2 << num2;
  std::string ipstr1 = ss1.str(), ipstr2 = ss2.str();
  char *ipchar1 = const_cast<char*>(ipstr1.c_str()), *ipchar2 = const_cast<char*>(ipstr2.c_str());

  addr[0] = ns3::Ipv4Address (ipchar1);
  addr[1] = ns3::Ipv4Address (ipchar2);
}

int get_add_delay_topoFaker(int pkt_count){//fake topo
  if(!is_init){
    std::stringstream ss;
    ss << "/home/myp4/Desktop/fake_delay_exp";
    ss << experiment_index;
    ss << "/topofaker/";
    if(TOPO_INDEX == 1){
      ss << "dfn/sort_res/dfn_";
    }else if(TOPO_INDEX == 2){
      ss << "cogentco/sort_res/cogentco_";
    }else{
      ss << "deltacom/sort_res/deltacom_";    
    }
    ss << ((precent_index+1)*10) << ".txt";
    file.open(ss.str());
    is_init = true;
  }
  if(file.is_open()) {
...
int get_add_delay(int pkt_count){
  if(fake_delay_mode == 1)
    return get_add_delay_topoFaker(pkt_count);
  return 0;
}
...
void
PointToPointNetDevice::SetExperimentInfo(int topo_index, int prec_index, int exp_index, int add_delay_type){
  TOPO_INDEX = topo_index;
  precent_index = prec_index;
  experiment_index = exp_index;
  if(add_delay_type < 0)
    add_fake_delay = 0;
  else if(add_delay_type > 0){
    add_fake_delay = 1;
    fake_delay_mode = add_delay_type;
  }
}

ns3::Ipv4Address preFrom, preTo;
int pkt_count = 0;
...
PointToPointNetDevice::TransmitStart (Ptr<Packet> p)
{
  ...
  Ptr<Packet> addr_copy = p->Copy();
  PppHeader pppheader;
  addr_copy->RemoveHeader(pppheader);
  Ipv4Header ipheader;
  addr_copy->RemoveHeader(ipheader);

  ns3::Ipv4Address address1 = ipheader.GetSource();
  ns3::Ipv4Address address2 = ipheader.GetDestination();

  MyTag tag;
  if(p->PeekPacketTag(tag)){
    uint32_t tag_val = tag.GetSimpleValue();
    if((tag_val & 0x02000000) == 0){
      if(!address1.IsEqual(preFrom) || !address2.IsEqual(preTo)){
        preFrom = address1;
        preTo = address2;
        std::cout << std::endl;
        std::cout << "mypathes\t" << address1 << "\t" << address2 << "\t" <<  this->GetNode()->GetId();
      }else
  ...
  if(add_fake_delay == 1 && p->PeekPacketTag(tag)){
    uint32_t tag_val = tag.GetSimpleValue();
    if((tag_val & 0x01000000) == 0){//add delay
      ns3::Ipv4Address need_addr[2];
      get_source_and_dest_addr(pkt_count, need_addr);
      //std::cout <<  "pkt info\t" << pkt_count << "\t" << need_addr[0] << "\t " << address1.IsEqual(need_addr[0]) << "\t" << need_addr[1]  << "\t"  << address2.IsEqual(need_addr[1]) << "\t" << (p->GetSize()==pkt_base_len) << std::endl;
      if(ipheader.GetTtl() == 64 && address1.IsEqual(need_addr[0]) && address2.IsEqual(need_addr[1]) && p->GetSize() == pkt_base_len){
        int need_add_delay = get_add_delay(pkt_count);
        if(need_add_delay >= one_hop_delay)
          txTime += MicroSeconds(need_add_delay - one_hop_delay);
        //std::cout << "add delay: " << need_addr[0] << " to " << need_addr[1] << " add " <<  need_add_delay << ", pkt_count : " << pkt_count << std::endl;
        pkt_count++;
      }
    }
  }
