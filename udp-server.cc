          ...
          if (InetSocketAddress::IsMatchingType (from))
            {
              ...
              */
              ...
              std::string str(reinterpret_cast<const char*>(data), 10);
              MyTag tag;
              uint32_t tag_val = 0;
              if(packet->PeekPacketTag(tag))
                tag_val = tag.GetSimpleValue();
              if((tag_val & 0x04000000) == 0)
                NS_LOG_INFO("mySout:\t" << packet->GetUid() << "\t" << packet->GetSize() << "\t" << InetSocketAddress::ConvertFrom (from).GetIpv4 () << 
                              "\t" << str << "\t" << (Simulator::Now () - seqTs.GetTs ()));
              }
